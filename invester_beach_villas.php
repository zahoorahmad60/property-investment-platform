...
$pageTitle = "Invest in Beach Villas";

// Fetch beach villas from the database
$stmt = $conn->prepare("SELECT * FROM Property WHERE type = 'Beach_villas'");
$stmt->execute();
$beach_villas = $stmt->fetchAll(PDO::FETCH_ASSOC);
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    ...
    <main>
    <h1>Beach Villas Available for Investment</h1>
    <div class="property-container">
        <?php if (count($beach_villas) > 0): ?>
            <?php foreach ($beach_villas as $villa): ?>
                <?php
                // Fetch the number of investors for this villa
                ...
                $stmt_investors->bindParam(':property_ID', $villa['property_ID'], PDO::PARAM_INT);
                $stmt_investors->execute();
                $investors = $stmt_investors->fetch(PDO::FETCH_ASSOC);
                $num_investors = $investors['num_investors'] ?? 0;

                // Fetch total amount invested in this villa
                ...
                $stmt_investment->bindParam(':property_ID', $villa['property_ID'], PDO::PARAM_INT);
                ...
                $percentage_funded = ($total_invested / $villa['cost_of_property']) * 100;
                ?>

                <div class="property-card" style="position: relative;">
                    <img src="<?php echo str_replace('\\', '/', 'seller/' . htmlspecialchars($villa['image_path'])); ?>" alt="<?php echo htmlspecialchars($villa['name']); ?>">
                    ...
                    <div class="property-info">
                        <h2> <a href="invester_property_details.php?property_id=<?php echo htmlspecialchars($villa['property_ID']); ?>" class="btn">
                                    <?php echo htmlspecialchars($villa['name']); ?>
                                </a></h2>
                        <p><strong>Price:</strong> $<?php echo number_format($villa['cost_of_property'], 2); ?></p>
                        <p><strong>Location:</strong> <?php echo htmlspecialchars($villa['city']); ?></p>
                    ...
                    <div class="button-group">
                        <?php if ($percentage_funded < 100): ?>
                            <?php if (isset($_SESSION['username']) && $_SESSION['user_type'] === 'investor'): ?>
                                <button class="invest-button" onclick="location.href='invester_invest_property.php?property_id=<?php echo $villa['property_ID']; ?>'">Invest Now</button>
                            <?php else: ?>
            ...
            <?php endforeach; ?>
        <?php else: ?>
            <p>No beach villas available for investment at the moment.</p>
        <?php endif; ?>
    </div>
</main>
