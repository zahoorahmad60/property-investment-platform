...
$noNavbar = true;
include 'init.php'; // Ensure this file sets up the database connection correctly
include 'admin_sidebar.php';

// Fetch property type from the URL to load the specific page
$type = isset($_GET['type']) ? $_GET['type'] : 'Apartments';

$pageTitle = ucfirst($type) . " Listings";

// Fetch properties and associated seller information based on type and calculate admin fee
try {
    $stmt = $conn->prepare("
        SELECT p.property_ID, p.name AS property_name, p.city, p.image_path, p.type, 
               p.cost_of_property, 
    ...
    // Debugging: Capture and display the error
            ...
            <tbody>
                <?php foreach ($properties as $property): ?>
                    <tr>
                        ...
                        <td><?php echo number_format($property['admin_fee'], 2); ?></td>
                        <td><img src="seller/<?php echo htmlspecialchars($property['image_path']); ?>" alt="Property Photo" style="width: 100px; height: auto;"></td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
...
<script>
    function showSellerProfile(property) {
        const { Fname, Lname, city, type } = property;
        alert(`Seller: ${Fname} ${Lname}\nCity: ${city}\nType: ${type}`);
    }
</script>
</body>
</html>
